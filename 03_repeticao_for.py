...
def soma_dos_inteiros(valor1, valor2):
    """
    Calcule a soma dos inteiros no intervalo dado.
    ...
    # soma = 0
    # if valor1 < valor2:
    #     for valor in range(valor1+1, valor2):
    #         soma += valor
    # else:
    #     for valor in range(valor2+1, valor1):
    #         soma += valor
...
def dar_troco(valor_a_pagar, valor_em_dinheiro):
    ...
    for nota in notas:
        ...
        troco = troco % nota
        if qt_notas > 0:
            lista_troco.append((nota, qt_notas))
...
def media_anual(temperaturas):
    ...
    que possuem temperatura superior à média anual.
...
def altera_salarios(salarios):
    ...
    for i, salario in enumerate(salarios):
        if salario <= SM: percentual_aumento = 20
        elif salario <= 2 * SM: percentual_aumento = 15
        elif salario <= 5 * SM: percentual_aumento = 10
        else: percentual_aumento = 5

        aumento = salario * percentual_aumento / 100
        novo_salario = salario + aumento

        salarios[i] = novo_salario
...
def main():
    ...
    test(soma_temperaturas([]), 0)
    test(soma_temperaturas([0]), 0)
    test(soma_temperaturas([1]), 1)
    test(soma_temperaturas([1, 1]), 2)
    test(soma_temperaturas([2, 3]), 5)
    test(soma_temperaturas([1, 3, 2, 5]), 11)
    test(soma_temperaturas([-1, 1]), 0)
    test(soma_temperaturas([10, -10]), 0)
    test(soma_temperaturas([-2, -1]), -3)
